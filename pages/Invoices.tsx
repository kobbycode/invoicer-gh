...
const Invoices: React.FC = () => {
  ...
  const { invoices, isLoading: loading, deleteInvoice, updateInvoiceStatus } = useInvoices();
  ...
  const handleMarkAsPaid = async (invoice: Invoice) => {
    ...
    try {
      ...
      await updateInvoiceStatus({ id: invoice.id, status: InvoiceStatus.PAID });
      ...
      // Query invalidation will handle the UI update
    ...
    <div className="space-y-8">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button
          ...
          variant="secondary"
          leftIcon={<span className="material-symbols-outlined mr-2 text-base sm:text-lg">file_download</span>}
        >
          Export CSV
        </Button>
      </header>
        ...
        <div className="p-4 flex flex-col md:flex-row justify-between gap-4 border-b border-gray-100 dark:border-gray-700">
          <div className="flex gap-2 p-1 bg-gray-50 dark:bg-gray-900 rounded-xl w-fit">
            {['All', 'Paid', 'Pending'].map((f) => (
              <button
                ...
                className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${filter === f ? 'bg-white dark:bg-gray-800 shadow-sm text-primary' : 'text-gray-500 hover:text-gray-700'}`}
              >
                {f}
              </button>
            ))}
          ...
          <div className="relative w-full md:w-96">
            ...
            <input
              ...
              className="w-full h-11 bg-gray-50 dark:bg-gray-900 border-none rounded-xl pl-10 text-sm focus:ring-2 focus:ring-primary/20 focus:text-gray-900"
                        ...
                        <div className="flex items-center justify-end gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                          ...
                          {inv.status !== InvoiceStatus.PAID && (
                            <Button
                              ...
                              variant="ghost"
                              size="sm"
                              className="text-xs bg-green-50 text-green-700 hover:bg-green-100 h-8"
                            >
                              {actionLoading === inv.id ? '...' : 'Mark Paid'}
                            </Button>
                          )}
                          <button onClick={() => handleDelete(inv.id)} className="p-2 hover:bg-red-50 text-gray-400 hover:text-red-500 rounded-lg transition-colors" title="Delete">
          ...
          <AnimatePresence>
            {loading ? (
              ...
              <motion.div
                ...
                transition={{ delay: index * 0.05 }}
                key={inv.id}
                className="p-4 flex flex-col gap-4 active:bg-gray-50 transition-colors"
              >
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <Button onClick={() => navigate(`/invoices/${inv.id}/edit`)} variant="secondary" className="flex-1 text-amber-600 bg-amber-50" leftIcon={<span className="material-symbols-outlined text-lg">edit</span>} />
                  ...
                  <Button
                    ...
                    variant="secondary"
                    className="flex-1 text-green-600 bg-green-50 border-transparent hover:bg-green-100"
                    leftIcon={<span className="material-symbols-outlined text-lg">send</span>}
                  />
                  ...
                  <Button onClick={() => handleDelete(inv.id)} variant="destructive" className="flex-1 bg-red-50 text-red-600 shadow-none border-transparent hover:bg-red-100" leftIcon={<span className="material-symbols-outlined text-lg">delete</span>} />
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
